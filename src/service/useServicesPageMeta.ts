...
import { servicesSeo } from "@/service/content";
...
export function useServicesPageMeta() {
  useEffect(() => {
    ...
    document.title = servicesSeo.title;

    const description = upsertMeta("name", "description", servicesSeo.description);
    const ogTitle = upsertMeta("property", "og:title", servicesSeo.title);
    const ogDescription = upsertMeta("property", "og:description", servicesSeo.description);
