...
export async function fetchPublishedPostBySlug(slug: string): Promise<BlogPostPublic | null> {
  ...
  if (data) {
    return mapRowToPublic(data as PostRow);
  }
...
const adminSelect = `
  ...
  post_tags ( blog_tags ( id, name, slug ) )
`;
...
export async function adminFetchPostBySlug(slug: string): Promise<AdminPostRow | null> {
  const { data, error } = await supabase
    .from("posts")
    .select(adminSelect)
    .eq("slug", slug)
