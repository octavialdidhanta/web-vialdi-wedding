 ...
 *
 * Why: some crawlers (notably WhatsApp) can be flaky fetching Supabase Storage URLs directly.
...
export default async function handler(request: Request): Promise<Response> {
  ...
  const imgRes = await fetch(imageUrl, {
    // Crawlers are sensitive to slow responses; hint cache where possible.
    headers: { Accept: "image/*" },
  });

  if (!imgRes.ok) {
    // Fall back to a stable site asset.
  ...
  return new Response(imgRes.body, {
    ...
    headers: {
      ...
      // Cache aggressively at edge; update will reflect once slug cover changes + cache expires.
      "cache-control": "public, max-age=86400",
    },
  });
}
