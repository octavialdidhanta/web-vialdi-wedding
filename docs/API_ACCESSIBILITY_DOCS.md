...
> v**1.4.9** · `npm run generate:omnichannel-api-docs`
...
1. **Legacy** — `organization_whatsapp_templates.template_language` (jika baris aktif untuk `web_id` + `template_name`).
2. **Org setting** — `default_whatsapp_lead_template_language`.
3. **Fallback** — `id`.

### Mapping variabel template lead

Synckerja memilih mapping **otomatis**:

1. **Legacy (prioritas)** — jika ada baris `organization_whatsapp_templates` aktif untuk pasangan `web_id` token SDK + `template_name` = setting org → pakai `body_keys` (koma) + `template_language` dari baris itu.
2. **Default (fallback)** — jika baris legacy tidak ada → **7 variabel body** tetap (urutan di bawah).

**Contoh legacy `vialdi-wedding` + `elementorform`:**
...
Email, telepon, dan alamat **tidak** dikirim ke template 5-slot ini (bukan bug website).

**Fallback fixed 7 variabel** (website baru tanpa baris legacy):
