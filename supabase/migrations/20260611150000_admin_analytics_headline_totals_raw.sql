-- Fix acquisition channel mislabel: if landing_url contains utm params but parsed utm_* columns are empty,
-- treat as Campaign (UTM) instead of Direct.
--
-- This complements client-side case-insensitive UTM parsing.
...
comment on function public.admin_analytics_summary(timestamptz, timestamptz, text) is
  'CMS dashboard: aggregates for one web_id. Human-filtered page views; channel classification treats landing_url containing utm_ as Campaign (UTM) to avoid mislabel as Direct when UTM parsing fails.';
