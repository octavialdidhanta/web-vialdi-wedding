-- CMS fresh install: agency_* tables with RLS policies created AFTER all tables exist.
-- Fixes forward-reference bug in supabase/migrations/20260424131000_agency_posts.sql

drop table if exists public.agency_post_tags cascade;
drop table if exists public.agency_posts cascade;
drop table if exists public.agency_blog_tags cascade;
drop table if exists public.agency_blog_categories cascade;

create table public.agency_blog_categories (
...
create table public.agency_blog_tags (
  id uuid primary key default gen_random_uuid(),
  slug text not null unique,
  name text not null,
  created_at timestamptz not null default now(),
...
create table public.agency_posts (
  id uuid primary key default gen_random_uuid(),
  slug text not null unique,
  title text not null,
...
create index idx_agency_posts_category on public.agency_posts(category_id);
create index idx_agency_posts_status_published on public.agency_posts(status, published_at desc nulls last);

create table public.agency_post_tags (
...
create index idx_agency_post_tags_tag on public.agency_post_tags(tag_id);
